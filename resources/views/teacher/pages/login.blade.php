...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'login') {
    ...
    if ($username === '' || $password === '') {
        ...
        $result = portal_authenticate_user($username, $password, ['teacher']);

        if (!($result['success'] ?? false)) {
            $error = $result['message'] ?? 'Unable to sign in right now.';
        } else {
            $teacher = get_teacher_profile_by_user_id((int) $result['user']['id']);

            if (!$teacher) {
                $error = 'Your teacher profile could not be found.';
            } else {
                session_regenerate_id(true);
                $_SESSION['teacher_user_id'] = (int) $result['user']['id'];
                $_SESSION['teacher_id'] = (int) $teacher['id'];
                $_SESSION['teacher_username'] = $result['user']['username'];
              ...
              <div class="card card-profile">
                ...
                <div class="card-body">
                  <div>
                    ...
                    <hr>
                    <p class="text-center">Sign in to manage your classes, attendance, and scores.</p>
                  </div>
                  <?php if ($error !== ''): ?>
                  ...
                  <form method="POST">
                    ...
                    <button type="submit" class="btn btn-primary w-100">Login to Teacher Portal</button>
                  </form>
                  <div class="text-center mt-3">
                    <small class="text-muted">Sample teacher password: <strong>password</strong></small>
                  </div>
                </div>
              </div>
            </div>
