...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($first_name) || empty($last_name) || empty($date_of_birth) || empty($gender) || empty($class_link) || empty($academic_session_link) || empty($admission_date)) {
        ...
        try {
            ...
            $email = $username . '@student.fimocol.edu.ng'; // Generate a school email
            ...
            $stmt = $pdo->prepare(
                'INSERT INTO students (user_link, admission_no, first_name, last_name, other_names, date_of_birth, gender, state_of_origin, lga, home_address, class_link, academic_session_link, admission_date, student_type, blood_group, genotype, status) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
            );
            $stmt->execute([
                $user_id, $admission_no, $first_name, $last_name, $other_names, $date_of_birth, $gender, $state_of_origin, $lga, $home_address, $class_link, $academic_session_link, $admission_date, $student_type, $blood_group, $genotype, $status
            ]);
                        ...
                        <div class="form-group">
                          ...
                          <select class="form-control" name="academic_session_link" id="academic_session_link" required>
                            ...
                            <option value="<?php echo $session['id']; ?>">
                              <?php echo htmlspecialchars($session['session_name']).' ('.$session['session_term'].' Term)'; ?></option>
                            <?php endforeach; ?>
                          </select>
                        </div>
                      </div>
