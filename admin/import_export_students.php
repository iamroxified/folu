        ...
        while (($data = fgetcsv($handle, 1000, ',')) !== FALSE) {
            try {
                ...
                if (!empty($firstName) && !empty($lastName) && filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    ...
                    $stmt = $pdo->prepare('INSERT INTO students (student_number, first_name, last_name, email, status) VALUES (?, ?, ?, ?, ?)');
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    $students = QueryDB("SELECT student_number, first_name, last_name, email, status, created_at FROM students")->fetchAll();
    
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename="students_export_' . date('Y-m-d') . '.csv"');
    
    $output = fopen('php://output', 'w');
    fputcsv($output, ['Student Number', 'First Name', 'Last Name', 'Email', 'Status', 'Created At']);
